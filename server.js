...
exports.default = _mongoose2.default.model('entry', channelSchema);
...
var _koaBody = __webpack_require__(44);
...
var _paragraphs = __webpack_require__(46);
...
var _mongoConnect = __webpack_require__(152);
...
try {
  _mongoose2.default.connect(_mongoConnect2.default.connection);
...
exports.default = new _koa2.default().use((0, _cors2.default)()).use((0, _koaBody2.default)()).use(_paragraphs2.default.routes()).use(_entries2.default.routes()).listen(PORT, () => console.log(`Backend service listen on port: ${PORT}`));
...
var _paragraphs = __webpack_require__(47);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

exports.default = new _koaRouter2.default().get('/paragraphs', _paragraphs.List);
...
var _jsdom = __webpack_require__(49);
...
var _mapKeys = __webpack_require__(51);
...
const List = exports.List = async ctx => {
  try {
    ...
    try {
      const result = await _axios2.default.get(articleURL);
      ...
      try {
        ...
        const paragraphs = [];
        (0, _mapKeys2.default)(paragraphsBuffer, k => paragraphs.push(k.textContent));
...
var _entries = __webpack_require__(134);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

exports.default = new _koaRouter2.default().get('/entries', _entries.List).post('/entries', _entries.Create).patch('/entries/:id', _entries.Patch).delete('/entries/:id', _entries.Delete);
...
var _Patch = __webpack_require__(144);
...
var _Delete = __webpack_require__(151);
...
var _sha = __webpack_require__(136);
...
var _v = __webpack_require__(137);

var _v2 = _interopRequireDefault(_v);

var _entryValidator = __webpack_require__(141);

var _entryValidator2 = _interopRequireDefault(_entryValidator);
...
const Create = exports.Create = async ctx => {
  try {
    ...
    const { isValid, errors } = (0, _entryValidator2.default)({ articleURL, originalText, usersText });
    if (!isValid) {
      ctx.throw(400, { message: errors });
    }

    const id = (0, _sha2.default)(`${articleURL}${originalText}`);
    const ext = await _entry2.default.findOne({ id });
    ...
    if (!ext) {
      ...
      if (!(await (0, _entry2.default)(entry).save())) {
      ...
      ext.suggestions.push({ id: (0, _v2.default)(), text: usersText });
      if (!(await ext.save())) {
        ctx.throw('409', { message: 'Conflict create entry' });
      } else {
        ctx.status = 200;
...
var rng = __webpack_require__(138);
...
var _validator = __webpack_require__(142);
...
exports.default = ({ articleURL, originalText, usersText }) => {
  const errors = {};
...
var _findIndex = __webpack_require__(145);

var _findIndex2 = _interopRequireDefault(_findIndex);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

const Patch = exports.Patch = async ctx => {
  try {
    const { id } = ctx.params;
    ...
    const entry = await _entry2.default.findOne({ id });
    if (!entry) {
      ctx.throw(404, { message: 'Entry not found!' });
    }

    const newSuggestions = entry.suggestions.map(({ id, text }) => ({ id, text }));
    const index = (0, _findIndex2.default)(newSuggestions, suggestion => suggestion.id === suggestionId);
    if (index < 0) {
...
const List = exports.List = async ctx => {
  try {
    ...
    const entries = await _entry2.default.find({ isApproved: !!showApproved });
    if (!entries) {
...
const Delete = exports.Delete = async ctx => {
  try {
    const { id } = ctx.params;

    if (!(await _entry2.default.remove({ id }))) {
